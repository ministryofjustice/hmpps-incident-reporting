...
describe('formGetRoute', () => {
  ...
  it('only allows submission with known formId', () => {
    ...
    return request(app)
      .get('/')
      .query({ formId: 'wrong-form', surname: 'Davies' })
      .expect(400)
      .expect((res: Response) => {
        expect(res.text).not.toEqual('DONE')
      })
  ...
  describe('allows handler to access submitted form', () => {
    it('when payload was valid', () => {
      ...
      return request(app)
        .get('/')
        .query({ formId: 'sample-form', surname: 'Davies' })
        .expect(200)
        .expect(res => {
          expect(res.text).toEqual('DONE')
    ...
    it('when payload was invalid', () => {
      ...
      return request(app)
        .get('/')
        .query({ formId: 'sample-form', name: 'John' })
        .expect(200)
        .expect(res => {
          expect(res.text).toEqual('DONE')
          expect(submittedForm).toBeInstanceOf(SampleForm)
