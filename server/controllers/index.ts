...
> extends FormWizard.Controller<V, K> {
  ...
  /**
   * Builds an object of values for all fields and steps from session or, alternatively, from request form values.
   * Values for fields with unmet dependencies are discarded.
   * Validation is not performed.
   */
  protected getAllValues(req: FormWizard.Request<V, K>, fromSession = true): V {
      ...
      if (Object.hasOwn(allFields, fieldName)) {
        allValues[fieldName] = fromSession
          ...
          : // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore not all fieldName will be present
